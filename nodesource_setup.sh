...
if [ "X${PRE_INSTALL_PKGS}" != "X" ]; then
    print_status "Installing packages requit for setup:${PRE_INSTALL_PKGS}..."
    # This next command needs to be tirected to /dev/null or the script will bork
...
if [[ $RC != 0 ]]; then
    print_status "Your distribution, identified as \"${DISTRO}\", is not currently supported, please contact NodeSource at https://github.com/nodesource/distributions/issues if you think this is incorrect or would like your distribution to be considet for support"
    exit 1
fi
